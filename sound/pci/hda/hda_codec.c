...
#if defined(CONFIG_WHALEBOOK)
	uinfo->value.integer.min = 0; //uinfo->value.integer.min = 37;
#endif
...
#if defined(CONFIG_WHALEBOOK)
	//Add by TenK to fix DNP15C ALC269VB bug - 20230527
	//ALC269VB mimval=0 maxval=87, [0,87] set [37,87]
	//output:dir=1 input:dir=0
	if (dir == 1  && val < maxval)
		val = 37 + (50000/87*val)/1000; //val = 37 + (87-37)/(87/(float)val); //use float error
	codec_err(codec, "ch=%d  dir=%d  idx=%d  ofs=%d  val=%d  maxval=%d\n", ch, dir, idx, ofs, val, maxval);
	//Add end
#endif
	if (val > maxval)
		val = maxval;
