...
static const char *const lid_angle_labels[CHANNEL_SCAN_INDEX_MAX] = { "lidangle", };
			  ...
			  long mask)
{
	...
	*val2 = 0;
	...
	case IIO_CHAN_INFO_RAW:
		...
		if (report_id < 0) {
			hid_sensor_power_state(&lid_state->common_attributes, false);
			ret_type = -EINVAL;
		}
        else {
			*val = sensor_hub_input_attr_get_raw_value(lid_state->common_attributes.hsdev,
							   hsdev->usage,
							   lid_angle_addresses[chan->scan_index],
							   report_id,
							   SENSOR_HUB_SYNC, min < 0) / 10;
	...
	case IIO_CHAN_INFO_SAMP_FREQ:
		ret_type =  hid_sensor_read_samp_freq_value(
		    &lid_state->common_attributes, val, val2);
        break;
				...
				char *raw_data, void *priv)
{
	...
	case HID_USAGE_SENSOR_TIME_TIMESTAMP:
		lid_state->timestamp = hid_sensor_convert_timestamp(&lid_state->common_attributes,
							     *(int64_t *)raw_data);
		ret = 0;
    break;
	default:
...
static int lid_angle_parse_report(struct platform_device *pdev,
			      ...
			      unsigned int usage_id, struct lid_angle_state *lid_state)
{
	int ret;
	int i;
	...
	for (i = 0; i < CHANNEL_SCAN_INDEX_MAX; ++i) {
		ret = sensor_hub_input_get_attribute_info(hsdev,
							  HID_INPUT_REPORT,
							  usage_id,
							  lid_angle_addresses[i],
							  &lid_state->lid_angle[i]);
		if (ret < 0)
			return ret;
		lid_angle_adjust_channel_bit_mask(channels, i, lid_state->lid_angle[i].size);
	}
	lid_state->scale_precision = hid_sensor_format_scale(
	        HID_USAGE_SENSOR_HINGE,
			&lid_state->lid_angle[CHANNEL_SCAN_INDEX_LID_ANGLE],
			&lid_state->scale_pre_decml, &lid_state->scale_post_decml);
...
static int lid_angle_probe(struct platform_device *pdev)
{
	...
	indio_dev->channels = devm_kmemdup(&indio_dev->dev, lid_angle_channels,
					   sizeof(lid_angle_channels), GFP_KERNEL);
